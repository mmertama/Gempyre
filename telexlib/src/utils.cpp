...
public:
    ...
    std::ostream& stream(TelexUtils::LogLevel logLevel) {
...
static TelexUtils::LogLevel g_serverLogLevel = TelexUtils::LogLevel::
...
void TelexUtils::init() {
#ifdef WINDOWS_OS
	if(!LoadLibraryA("ntdll.dll")) {
        TelexUtils::log(TelexUtils::LogLevel::Warning, "Cannot preload", "ntdll.dll");
	}
	if(!LoadLibraryA("kernel32.dll")) {
        TelexUtils::log(TelexUtils::LogLevel::Warning, "Cannot preload", "kernel32.dll");
	}
	if(!LoadLibraryA("advapi32.dll")) {
        TelexUtils::log(TelexUtils::LogLevel::Warning, "Cannot preload", "advapi32.dll");
	}
#endif
}
...
void TelexUtils::setLogLevel(TelexUtils::LogLevel level, bool useSysLog) {
...
DebugStream TelexUtils::logStream(LogLevel logLevel) {
#ifdef WINDOWS_OS
    (void) logLevel;
#endif
    auto& strm =
...
TelexUtils::LogLevel TelexUtils::logLevel() {
    return g_serverLogLevel;
}

bool TelexUtils::useSysLog() {
#ifndef WINDOWS_OS
...
std::variant<std::tuple<std::multimap<std::string, std::string>, std::vector<std::string>>, int> TelexUtils::parseArgs(int argc, char* argv[], const std::initializer_list<std::tuple<std::string, char, ArgType>>& args) {
#ifndef WINDOWS_OS
...
std::string TelexUtils::absPath(const std::string& rpath) {
#ifndef WINDOWS_OS
...
#ifndef WINDOWS_OS
std::string TelexUtils::readProcess(const std::string& processName) {
    auto fd = ::popen(processName.c_str(), "r");
    telex_utils_assert_x(fd, "cannot create pipe");
    telex_utils_auto_close(fd, ::pclose);
    std::string out;
    char buffer[256];
...
std::string TelexUtils::tempName() {
#ifndef USE_TEMPNAM
...
void TelexUtils::removeFile(const std::string& name) {
    ::remove(name.c_str());
}


std::string TelexUtils::substitute(const std::string& str, const std::string& substring, const std::string& subsitution) {
    try {
    return std::regex_replace(str, std::regex(substring) , subsitution);
    } catch (std::regex_error& e) {
         log(LogLevel::Error, "reg exp", substring, e.code());
         telex_utils_assert_x(false, "regexp");
    }
    return std::string();
}

#ifdef UNIX_OS //fix if needed
std::string TelexUtils::appPath() {
    return getLink("/proc/self/exe");  //obviously UNIX only
}
#endif


bool TelexUtils::fileExists(const std::string& filename) {
#ifndef WINDOWS_OS
...
std::vector<std::tuple<std::string, bool, std::string>> TelexUtils::directory(const std::string& dirname) {
    ...
    while(auto dirEntry = readdir(dir)) {
        if(dirEntry->d_type == DT_LNK) {
            ...
            const auto linked = TelexUtils::getLink(fullname);
            const auto fulllink = dname + linked;
            entries.push_back({dirEntry->d_name, TelexUtils::isDir(fulllink), fulllink});
        } else
...
#ifndef WINDOWS_OS
bool TelexUtils::isDir(const std::string& path) {
   struct stat statbuf;
...
std::string TelexUtils::getLink(const std::string& lname) {
 #ifndef WINDOWS_OS
    size_t sz = 128;
    for(;;) {
...
std::string TelexUtils::systemEnv(const std::string& env) {
    if(env.length() > 0) {
...
std::string TelexUtils::hostName() {
#ifndef WINDOWS_OS
    char buf[256] = {0};
...
bool TelexUtils::isHiddenEntry(const std::string& filename) {
#ifndef WINDOWS_OS
    char s[4096] = {0};
...
#ifdef UNIX_OS
long TelexUtils::timeStamp(const std::string& filename) {
    struct stat attr;
...
bool TelexUtils::rename(const std::string& of, const std::string& nf) {
...
#ifndef WINDOWS_OS
std::string TelexUtils::workingDir() {
    size_t sz = 128;
    for(;;) {
        std::vector<char> buffer(sz);
...
std::string TelexUtils::slurp(const std::string& file, size_t max) {
...
std::string TelexUtils::hexify(const std::string& src, const std::string pat) {
    std::regex rx(pat);
    ...
    for (auto i = words_begin; i != words_end; ++i) {
       ...
       telex_utils_assert_x(i->str().length() == 1, "Bad pattern, should only match a single char");
...
std::string TelexUtils::unhexify(const std::string& src) {
    std::string out;
...
bool TelexUtils::isExecutable(const std::string& filename) {
...
SSIZE_T TelexUtils::fileSize(const std::string& filename) {
...
std::string TelexUtils::which(const std::string& filename) {
...
std::shared_ptr<TelexUtils::expiror> TelexUtils::waitExpire(std::chrono::seconds s, const std::function<void ()>& onExpire) {
...
TelexUtils::OS TelexUtils::currentOS() {
#if defined(MAC_OS)
    return OS::MAC;
...
std::string TelexUtils::currentTimeString() {
    const auto result = std::time(nullptr);
    char timebuf[64];
    strftime(timebuf, sizeof timebuf, "%c", std::localtime(&result));
    return TelexUtils::chop(timebuf);
}

#ifdef UNIX_OS //fix if needed
bool TelexUtils::setPriority(int priority) {
      sched_param sch;
      int thisPolicy;
...
UTILS_EX std::pair<int, int> TelexUtils::getPriorityLevels() {
