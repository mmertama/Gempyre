...
std::optional<std::string> ContainertoString(const std::any& any) {
    if(const auto* v = std::any_cast<std::vector<T>>(&any)) {
        ...
        for(const auto& a : *v) {
            ...
            ++p;
            if(!o.has_value()) {
                 TelexUtils::log(TelexUtils::LogLevel::Error, "Vec", p);
                return std::nullopt;
            }
        ...
        for(const auto& [k, a] : *h) {
            const auto o = toString(a);
            if(!o.has_value()) {
                 TelexUtils::log(TelexUtils::LogLevel::Error, "Hash", k);
                return std::nullopt;
            }
        ...
        for(const auto& [k, a] : *h) {
            const auto o = toString(a);
            if(!o.has_value()) {
                TelexUtils::log(TelexUtils::LogLevel::Error, "Map", k);
                return std::nullopt;
            }
            obj.emplace(k, json::parse(o.value()));
        }
...
std::optional<std::string> Telex::toString(const std::any& any) {
    if(const auto* i = std::any_cast<int>(&any)) {
        ...
        if(v7.has_value())
            return v7;
        TelexUtils::log(TelexUtils::LogLevel::Error, "Invalid value:", any.type().name());
        return std::nullopt;
    }
}
