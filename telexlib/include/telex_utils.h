...
#define telex_utils_assert_x_f(b, x, f) (b || TelexUtils::doFatal(x, f, __FILE__, __LINE__))
#define telex_utils_fatal(x) TelexUtils::doFatal(x, nullptr, __FILE__, __LINE__)
#define telex_utils_fatal_f(x) TelexUtils::doFatal(x, f, __FILE__, __LINE__)
#define telex_utils_auto_clean(p, f) std::unique_ptr<std::remove_pointer<decltype(p)>::type, decltype(&f)> _ ## p (p, &f)
#define telex_utils_auto_close(p, f) TelexUtils::_Close<std::decay_t<decltype(p)>, decltype(&f)> _ ## p (p, &f)
...
inline void log(LogLevel level, const T& e) {
    if(useSysLog()) {
        ...
        if(level <= logLevel()) {
            logStream(level).print() << '[' << TelexUtils::currentTimeString() << "] " << toStr(level) << " " << e << std::endl;
...
std::string writeToTemp(const T& data) {
    const auto name = TelexUtils::tempName();
