 ...
 * 01.29	HVE/JAN-29-2017	Added support for table lookup of exp(1) as a transcendntal constant
 * 01.30	HVE/JUN-6-2017	Cleanup code by squashing some warning when compiling
 * 01.31    HVE/Jul-07-2019 Make the code more portable
 * 01.32	HVE/Jul-26-2019	Added the method .toFixed() with same functionality as the Javascript counterpart
...
inline int digits_to_check() { if (F_RADIX == BASE_10) return 18; if(F_RADIX == BASE_2) return 64; if (F_RADIX == BASE_256) return 8; if (F_RADIX == BASE_16) return 16; return 20; }
...
class float_precision {
   ...
   public:
	  ...
	  float_precision assign(const float_precision& a) { mRmode = a.mRmode; mPrec = a.mPrec; mExpo = a.mExpo; mNumber = a.mNumber; mSign = a.mSign;  return *this; }


	  int change_sign()								{ mSign *= -1;  return mSign; }
...
inline float_precision::float_precision( const char c, unsigned int p = float_precision_ctrl.precision(), enum round_mode m = float_precision_ctrl.mode() )
	...
	if( c < '0' || c > '9' )
		...
		mPrec = p;
		mSign = +1;	
		number = ito_precision_string( IDIGIT10( c ), false, F_RADIX );  // Convert to integer. can only be positive		                    
		number = number.substr( 1 );                       // Remove sign
		_float_precision_strip_leading_zeros( &number );   // First strip for leading zeros
		mExpo = (int)number.length() -1;                   // Always one digit before the dot
		_float_precision_strip_trailing_zeros( &number );  // Get rid of trailing non-significant zeros
		mExpo += _float_precision_rounding( &number, mSign, mPrec, mRmode );    // Perform rounding
		mNumber = number;								// Build number
		}
	}
...
inline float_precision::float_precision( const unsigned char c, unsigned int p = float_precision_ctrl.precision(), enum round_mode m = float_precision_ctrl.mode() )
	...
	if( c < '0' || c > '9' )
		...
		mPrec = p;
		mSign = +1;
		number = ito_precision_string( (unsigned long)IDIGIT10( (char)c ), false, F_RADIX );  // Convert to integer
		number = number.substr( 1 );                       // Remove sign
		_float_precision_strip_leading_zeros( &number );   // First strip for leading zeros
		mExpo = (int)number.length() -1;                   // Always one digit before the dot
		_float_precision_strip_trailing_zeros( &number );  // Get rid of trailing non-significant zeros
...
inline float_precision::float_precision( short i, unsigned int p = float_precision_ctrl.precision(), enum round_mode m = float_precision_ctrl.mode() )
	...
	mPrec = p;
	mExpo = 0;
	number = ito_precision_string( (unsigned long)i, true, F_RADIX ); // Convert to integer
	mSign = CHAR_SIGN( number[0] );;                    // Get Sign
	number = number.substr( 1 );                       // Remove sign
	_float_precision_strip_leading_zeros( &number );   // First strip for leading zeros
	mExpo = (int)number.length() -1;                   // Always one digit before the dot
	_float_precision_strip_trailing_zeros( &number );  // Get rid of trailing non-significant zeros
...
inline float_precision::float_precision( int i, unsigned int p = float_precision_ctrl.precision(), enum round_mode m = float_precision_ctrl.mode() )
	...
	number = ito_precision_string( (unsigned)i, true, F_RADIX );          // Convert to integer
	mSign = CHAR_SIGN( number[0] );;                    // Get Sign
	number = number.substr( 1 );                       // Remove sign
	_float_precision_strip_leading_zeros( &number );   // First strip for leading zeros
	mExpo = (int)number.length() -1;                   // Always one digit before the dot
...
inline float_precision::float_precision( int64_t i, unsigned int p = float_precision_ctrl.precision(), enum round_mode m = float_precision_ctrl.mode())
	...
	number = i64to_precision_string( (uint64_t)i, true, F_RADIX);          // Convert to integer
	mSign = CHAR_SIGN(number[0]);;                    // Get Sign
	number = number.substr(1);                       // Remove sign
	_float_precision_strip_leading_zeros(&number);   // First strip for leading zeros
	mExpo = (int)number.length() - 1;                // Always one digit before the dot
...
inline float_precision::float_precision( uint64_t i, unsigned int p = float_precision_ctrl.precision(), enum round_mode m = float_precision_ctrl.mode())
	...
	number = u64to_precision_string(i, F_RADIX);		// Convert to integer
...
inline float_precision& float_precision::operator+=( const float_precision& a )
	...
	if( a.mNumber.length() == 1 && FDIGIT( a.mNumber[0] ) == 0 )  // Add zero
		return *this;
	if( mNumber.length() == 1 && FDIGIT( mNumber[0] ) == 0 )      // Add a (not zero) to *this (is zero) Same as *this = a;
		return *this = a;

	// extract sign and unsigned portion of number
	sign1 = a.mSign;
	s1 = a.mNumber.substr();		// Extract Mantissa
	sign2 = mSign;
	s2 = mNumber.substr();			// Extract Mantissa
	...
	digits_max = (int)std::max( s1.length(), s2.length() );
	if( s1.length() != (unsigned)digits_max )
		_float_precision_left_shift( &s1, digits_max - (int)s1.length() );
	if( s2.length() != (unsigned)digits_max )
		_float_precision_left_shift( &s2, digits_max - (int)s2.length() );
...
inline bool operator==( const float_precision& a, const float_precision& b )
	...
	if( const_cast<float_precision&>(a).ref_mantissa()->length() == 1 && (*const_cast<float_precision&>(a).ref_mantissa())[0] == FDIGIT(0) && (*const_cast<float_precision&>(b).ref_mantissa())[0] == FDIGIT(0) )
...
inline bool operator<( const float_precision& a, const float_precision& b )
	...
	zero1 = const_cast<float_precision&>(a).ref_mantissa()->length() == 1 && FDIGIT( ( *const_cast<float_precision&>(a).ref_mantissa())[0] ) == 0 ? true : false;
	zero2 = const_cast<float_precision&>(b).ref_mantissa()->length() == 1 && FDIGIT( ( *const_cast<float_precision&>(b).ref_mantissa())[0] ) == 0 ? true : false;
...
///	@author Henrik Vestermark (hve@hvks.com)
///	@date  6/25/2014
///	@brief 	toFixed(fix)  float precision toFixed method
...
inline std::string float_precision::toFixed(int fix = 0)
	...
	if (expo > 0) 
		{ ss.insert(ss.length(), expo, '0'); inx += expo; }		// Trailing with zeros
	else if (expo < 0) ss.insert(0, -expo, '0');				// Padd with leading zeros
	_float_precision_rounding( &ss, sign == '-' ? -1 : 1, inx + fix, this->mRmode);  // Round to fix
...
///	@author Henrik Vestermark (hve@hvks.com)
///	@date  6/25/2014
///	@brief 	toPrecision(fix)  float precision toPrecision method
...
///	@author Henrik Vestermark (hve@hvks.com)
///	@date  6/25/2014
///	@brief 	toExponential(fix)  float precision toExponential method
